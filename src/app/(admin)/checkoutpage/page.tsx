...
function CheckoutPageContent() {
    ...
    async function fetchData() {
      try {
        ...
        for (const quotationData of quotationRows) {
          ...
          // Check if this quotation has an approved payment
          const { data: approvedPaymentData } = await supabase
            .from('payment_quotations')
            .select('payments!inner(status)')
            .eq('quotation_id', quotationData.id)
            .eq('payments.status', 'Approved')
            .limit(1)
          const hasApproved = !!(approvedPaymentData && approvedPaymentData.length > 0)
          ...
          formattedQuotations.push({
            ...
            client_label: quotationData.client_label ?? null,
            hasApprovedPayment: hasApproved,
          })
        ...
        if (isMounted) {
          ...
          formattedQuotations.forEach(q => { initialLabels[q.id] = q.client_label || '' })
  ...
  const handleCompletePayment = async () => {
    ...
    try {
      ...
      // Save labels for selected quotations (only if not already locked by approved payment)
      for (const q of selectedQuotationsList) {
        if (!q.hasApprovedPayment && q.uuid && labels[q.id] !== undefined) {
          await supabase
            .from('quotations')
            .update({ client_label: labels[q.id] || null } as never)
            .eq('id', q.uuid)
        }
      }
                  ...
                  <div className="p-4">
                    ...
                    <div className="mb-4">
                      <div className="flex items-center justify-between mb-1">
                        <label className="text-xs font-semibold text-[#0D47A1] flex items-center gap-1">
                        ...
                        {quotation.hasApprovedPayment && (
                          <span className="flex items-center gap-1 text-xs text-[#0D47A1] border border-[#0D47A1]/40 bg-[#E3F2FD] px-1.5 py-0.5 rounded-full">
                      ...
                      {quotation.hasApprovedPayment ? (
                        <div className="w-full rounded border border-[#0D47A1]/30 bg-[#E3F2FD] px-3 py-2 text-sm font-mono text-[#0D47A1]">
                          {labels[quotation.id] || '—'}
                        </div>
                      ) : (
                        <input
                          ...
                          className="w-full rounded border border-[#0D47A1]/40 bg-[#E3F2FD]/40 px-3 py-2 text-sm text-[#0D47A1] placeholder-blue-300 focus:outline-none focus:border-[#0D47A1] focus:ring-1 focus:ring-[#0D47A1]/30"
                        />
                      )}
                    </div>
